...
class WeightDependency:
    def __init__(self, id, form, tag, head, rel, weight):
        self.id = id
        self.org_form = form
        self.form = form.lower()
        self.tag = tag
    ...
    def __str__(self):
        values = [str(self.id), self.org_form, "_", self.tag, "_", "_", str(self.head), self.rel, "_", "_", self.weight]
        return '\t'.join(values)
...
def readWeightDepTree(file, vocab=None):
    proj = 0
    total = 0
    min_count = 1
    if vocab is None: min_count = 0
    if vocab is None: sentence = []
    else: sentence = [WeightDependency(0, vocab._root, vocab._root, 0, vocab._root, 1.0)]
    for line in file:
        tok = line.strip().split('\t')
        if not tok or line.strip() == '' or line.strip().startswith('#'):
            if len(sentence) > min_count:
            ...
            if vocab is None:
                ...
                sentence = [WeightDependency(0, vocab._root, vocab._root, 0, vocab._root, 1.0)]
        elif len(tok) == 11:
            if tok[6] == '_': tok[6] = '-1'
            try:
                sentence.append(WeightDependency(int(tok[0]), tok[1], tok[3], int(tok[6]), tok[7], float(tok[10])))
            except Exception:
                pass
        else:
            pass
    ...
    if gold == None:
        ...
        for idx in range(glength):
            ...
            values = [str(gold[start_g + idx].id), gold[start_g + idx].org_form, "_", gold[start_g + idx].tag, \
                      gold[start_g + idx].tag, "_", str(sentence[start_p + idx].head), sentence[start_p + idx].rel, "_", "_"]
            output.write('\t'.join(values) + '\n')

        output.write('\n')

def printDepTreeWeight(output, sentence, confidence, gold=None):

    num = 0
    for entry in sentence:
        if not entry.pseudo:
            output.write(str(entry) + '\t' + str(confidence[num]) + '\n')
            num +=1
    output.write('\n')
