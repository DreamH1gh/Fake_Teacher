...
class MBertExtractor(nn.Module):
    def __init__(self, config):
        super(MBertExtractor, self).__init__()
        self.config = config
        self.bert = MyBertModel.from_pretrained(config.mbert_path)
        self.bert.encoder.output_hidden_states = config.output_hidden_states
        self.bert.encoder.output_attentions = config.output_attentions
        self.bert_hidden_size = self.bert.config.hidden_size
        self.bert_layers = self.bert.config.num_hidden_layers + 1
...
class XLMRExtractor(nn.Module):
    def __init__(self, config):
        super(XLMRExtractor, self).__init__()
        self.config = config
        self.bert = XLMRobertaModel.from_pretrained(config.xlmr_path)
        self.bert.encoder.output_hidden_states = config.output_hidden_states
        self.bert.encoder.output_attentions = config.output_attentions
        self.bert_hidden_size = self.bert.config.hidden_size
        self.bert_layers = self.bert.config.num_hidden_layers + 1
    ...
    def forward(self, bert_indices, bert_segments, bert_pieces):
        ...
        # or idx in range(self.bert_layers):
            ...
            # outputs.append(cur_output)
        # return outputs
        output = torch.bmm(bert_pieces, all_outputs.last_hidden_state)
        return output
